...
def __scrap_pool__(doc: dict):
    ...
    try:
        for country in doc:
            ...
            if data:
                with open(f"data/html/tld/{country['tld']}.html", "w") as f:
                    f.write(data)
                print(f"{pname} {country['tld']} scraped")
            else:
                print(f"{pname} {country['tld']} not scraped")
...
def scrap():
    with ropen("data/json/report_dns.json", "r") as f:
        data = json.load(f)
    chunk_size = 10
